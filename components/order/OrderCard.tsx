    ...
    return (
        ...
        >
                ...
                {order.orderProducts.map(product => (
                    <div key={product.id} className="flex items-center gap-2 border-t border-gray-200 pt-4">
                        <dt className="flex items-center text-sm text-gray-600">
                            <span className="font-black">(x{product.quantity}){''}</span>
                        </dt>
                        <dd className="text-sm text-gray-900 font-medium">{product.product.name}</dd>
                    </div>
                ))}
            ...
            <form action={completeOrder}>
                ...
                <input
                    type="submit"
                    className="bg-indigo-600 hover:bg-indigo-700 transition-colors text-white w-full mt-5 p-3 uppercase font-bold cursor-pointer"
